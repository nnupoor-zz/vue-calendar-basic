<!-- Taken from https://github.com/icai/vue2-calendar/ and modified for custom usage -->
<template>
  <div class="datepicker">
    <template v-if="hasInput">
      <div class="form-control datepicker-input" :class="{'with-reset-button': clearButton}" type="text" :placeholder="placeholder"
          :style="inputStyle"
          @click="inputClick"
          v-model="inputValue" :epoch="epoch">{{inputValue}}</div>
    ...
    <div class="datepicker-popup" :style="paneStyle" @mouseover="handleMouseOver" @mouseout="handleMouseOver" v-show="displayDayView">
      <div class="datepicker-ctrl">
        <span class="rotate-left" aria-hidden="true" @click="preNextMonthClick(0)"><icon class="dd-icon" :id="'dropdown'" :width="13" :height="13"></icon></span>
        <span class="rotate-right" aria-hidden="true" @click="preNextMonthClick(1)"><icon class="dd-icon" :id="'dropdown'" :width="13" :height="13"></icon></span>
      ...
      <template v-for="(p, pan) in pane" >
        <div class="datepicker-inner">
          <div class="datepicker-body">
            <p class="reminder--month-name" @click="switchMonthView">{{stringifyDayHeader(currDate, pan)}}</p>
...
<script>
  ...
  function defaultText() {
        ...
        const date = ('0'+now.getDate()).slice(-2) //the format is dd/MM/yyyy
  ...
  export default {
    ...
    watch: {
      ...
      inputValue () {
        const epoch = new Date(this.currDate).getTime()
        // console.log('calendar', this.currDate, new Date(epoch), epoch)
        this.getEpochTime(epoch)
      },
    },
    computed: {
      text () {
    ...
    methods: {
      ...
      getDateRange () {
        ...
        for (let p = 0; p < this.pane; p++) {
          ...
          //Author: Chirag
          if (this.disablePrevDates) {
            var startDate = new Date(this.startDate)
            startDate.setHours(0,0,0,0)
            this.dateRange[p].forEach(day => {
              var isPrevDate = (((day.date.getTime() - startDate.getTime())/864000) < 0)
              if (isPrevDate) {
                day.sclass = 'datepicker-item-disable'
              }
            });
          }
        }
